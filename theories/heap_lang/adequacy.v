...
Proof.
  ...
  iModIntro. iExists
    (λ σ κs, (gen_heap_interp σ.(heap) ∗ trace_auth σ.(trace) ∗ proph_map_interp κs σ.(used_proph_id))%I),
    (λ _, True%I).
  iFrame. iApply (Hwp (HeapGS _ _ _ _ _)).
Qed.

Definition heap_invariance Σ `{!heapGpreS Σ} (N: namespace) (I: list val → Prop) s e σ :
  I (trace σ) →
  (∀ `{!heapGS Σ}, ⊢ trace_inv N I -∗ trace_is (trace σ) -∗ WP e @ s; ⊤ {{ _, True }}) →
  ∀ σ' t,
...
Proof.
  ...
  { iNext. eauto. }
  iModIntro. iExists
    (λ σ κs _, (gen_heap_interp σ.(heap) ∗ trace_auth σ.(trace) ∗ proph_map_interp κs σ.(used_proph_id))%I),
    (λ _, True%I).
  iFrame. iSplitL.
  { iDestruct (Hwp (HeapGS _ _ _ _ _)) as "Hwp". iApply ("Hwp" with "HI Ht"). }
...
Lemma module_invariance {Σ} `{heapGpreS Σ} (N: namespace)
  (Φ: ∀ `{heapGS Σ}, iProp Σ → val → iProp Σ)  (* Module specification *)
...
:
  ...
  (⊢ ∀ `(heapGS Σ) P m, Φ P m -∗ {{{ P }}} e m {{{ v, RET v; True }}}) →
  (* The initialization code must provide P0 *)
  (⊢ ∀ `(heapGS Σ), {{{ True }}} e_init {{{ v, RET v; P0 }}}) →
  ...
  (⊢ ∀ `(heapGS Σ), Φ (P0 ∗ trace_is (trace σ) ∗ trace_inv N good_trace)%I imimpl) →
...
Proof.
  ...
  iIntros (?) "HI Htr". wp_bind e_init. iApply Hinit. done.
